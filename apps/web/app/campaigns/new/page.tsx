  ...
  return (
    <div className="p-8 max-w-[800px] mx-auto space-y-8">
      <div className="pb-4 border-b border-border">
        ...
        <p className="text-sm text-muted-foreground mt-1">AI-assisted autonomous A/B testing campaign wizard.</p>
      </div>

      <div className="flex items-center gap-2 mb-8">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="flex items-center gap-2">
            <div className={`w-5 h-5 rounded flex items-center justify-center text-[10px] font-bold transition-colors ${step >= i ? 'bg-indigo-500 text-white shadow-[0_0_10px_rgba(99,102,241,0.5)]' : 'bg-muted text-muted-foreground border border-border'}`}>
              {step > i ? <Check className="w-3 h-3" /> : i}
            </div>
            {i < 4 && <div className={`w-8 h-[2px] transition-colors ${step > i ? 'bg-indigo-500/50' : 'bg-border'}`} />}
          </div>
        ))}
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            {segments.map((s) => (
              ...
              >
                <CardHeader className="border-b border-border pb-3 group-hover:border-indigo-500/20 transition-colors">
                ...
                <CardContent className="pt-4">
                  <CardDescription className="line-clamp-2 text-[12px] mb-4 group-hover:text-muted-foreground transition-colors">{s.description}</CardDescription>
          ...
          <Card>
            <CardContent className="pt-6 space-y-6">
              <div className="space-y-2">
                <label className="text-[11px] uppercase tracking-wider font-semibold text-muted-foreground">Campaign Goal</label>
                  <Input 
                    placeholder="e.g. Win back customers with a 20% discount" 
                    value={goal}
                    onChange={(e) => setGoal(e.target.value)}
                    className="bg-background focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500/30 transition-all hover:border-indigo-500/50"
                  />
              </div>
              <div className="p-4 bg-background rounded border border-border">
                <p className="text-[10px] text-muted-foreground uppercase font-semibold mb-2 tracking-wider">Targeting</p>
                <p className="text-[13px] font-medium">{segmentName}</p>
                <p className="text-[12px] text-muted-foreground mt-1">{segmentDescription}</p>
              </div>
            </CardContent>
          </Card>
      ...
      {step === 3 && (
        <div className="space-y-8">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Fine-tune the AI-generated copy. The system will automatically run multivariate tests on these.</p>
          </div>
          
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {variants.map((variant, idx) => {
              const themeColors = [
                { ring: 'focus-within:ring-border', border: 'focus-within:border-border', hover: 'hover:border-border hover:shadow-md', headerBg: 'group-focus-within:bg-muted/50', iconBg: 'bg-muted text-foreground border border-border shadow-sm', inputBorder: 'focus:border-border', text: 'group-focus-within:text-foreground' },
                { ring: 'focus-within:ring-border', border: 'focus-within:border-border', hover: 'hover:border-border hover:shadow-md', headerBg: 'group-focus-within:bg-muted/50', iconBg: 'bg-muted text-foreground border border-border shadow-sm', inputBorder: 'focus:border-border', text: 'group-focus-within:text-foreground' },
                { ring: 'focus-within:ring-border', border: 'focus-within:border-border', hover: 'hover:border-border hover:shadow-md', headerBg: 'group-focus-within:bg-muted/50', iconBg: 'bg-muted text-foreground border border-border shadow-sm', inputBorder: 'focus:border-border', text: 'group-focus-within:text-foreground' }
              ];
              ...
              return (
              <div key={idx} className={`flex flex-col bg-card border border-border rounded-lg overflow-hidden group focus-within:ring-2 transition-all duration-300 hover:-translate-y-1 ${theme.ring} ${theme.border} ${theme.hover}`}>
                <div className={`px-5 py-4 border-b border-border bg-card flex justify-between items-center transition-colors duration-300 ${theme.headerBg}`}>
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`text-[13px] font-medium text-foreground transition-colors ${theme.text}`}>Variant {String.fromCharCode(65 + idx)}</span>
                  </div>
                  <Badge variant="outline" className={`text-[10px] bg-transparent border-border text-muted-foreground transition-colors ${theme.text}`}>AI Generated</Badge>
                </div>
                
                <div className="p-5 space-y-6 flex-1 flex flex-col group-hover:bg-muted/5 transition-colors duration-300">
                  <div className="space-y-2">
                    ...
                    <Input 
                      ...
                      className={`bg-card border-border text-[14px] font-medium text-foreground shadow-sm transition-colors ${theme.inputBorder}`}
                    />
                  </div>
                  ...
                  <div className="space-y-2 flex-1 flex flex-col">
                    ...
                    <Textarea 
                      ...
                      className={`bg-card border-border min-h-[180px] text-[13px] text-muted-foreground resize-none flex-1 leading-relaxed shadow-sm transition-colors ${theme.inputBorder}`} 
                    />
                  </div>
                </div>
              </div>
            ...
            <Card className="border-border bg-card">
              <CardContent className="pt-5 flex gap-4 items-start">
                ...
                <div>
                  ...
                  <p className="text-[13px] text-muted-foreground mt-1">{recommendation.reason}</p>
                </div>
              </CardContent>
            </Card>
          )}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Card>
              <CardHeader className="border-b border-border">
                <CardTitle className="text-[12px] uppercase tracking-wider font-semibold text-muted-foreground">Configuration</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4 pt-4">
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Audience</p>
                  <p className="font-medium text-[13px]">{segmentName}</p>
                </div>
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Channel</p>
                ...
                <div>
                  <p className="text-[10px] text-muted-foreground uppercase font-semibold tracking-wider mb-1">Testing Mode</p>
            ...
            <Card>
              <CardHeader className="border-b border-border">
                <CardTitle className="text-[12px] uppercase tracking-wider font-semibold text-muted-foreground">Selected Variants</CardTitle>
              </CardHeader>
              <CardContent className="pt-4">
                <div className="p-4 bg-background rounded border border-border">
                  <p className="font-medium text-[13px] border-b border-border pb-2 mb-2">3 AI Variants generated</p>
                  <p className="text-[12px] text-muted-foreground">15% of the audience will be split among the variants. The highest performing variant will automatically be sent to the remaining 85%.</p>
                </div>
              </CardContent>
            </Card>
          </div>
